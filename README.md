...
- **Run segmentation training.** The file [thyroidiomics/train.py](thyroidiomics/trainddp.py) runs training on the 2D dataset via PyTorch's `DistributedDataParallel`. To run training, do the following (an example bash script is given in [./segmentation/train.sh](./segmentation/train.sh)):
    ```
